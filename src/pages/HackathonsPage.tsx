...
export function HackathonsPage() {
  ...
  const selectedTags = useMemo(() => {
    ...
    const source = tagsParam
      ? tagsParam.split(',')
      : legacyTag && legacyTag !== 'all'
        ? [legacyTag]
        : []
  ...
  const effectiveTagFilter = quickView === 'interest' ? [] : selectedTags

  const filtered = useMemo(() => {
    let rows = hackathons.filter((item) => {
      if (status !== 'all' && item.status !== status) return false
      if (effectiveTagFilter.length > 0 && !(item.tags || []).some((itemTag) => effectiveTagFilter.includes(itemTag))) return false
    ...
    return rows
  }, [detailsBySlug, effectiveTagFilter, hackathons, interestTags, keyword, nowMs, quickView, sortBy, status])
  ...
  const hasManualTagFilter = quickView !== 'interest' && selectedTags.length > 0

  const hasActiveFilter =
    Boolean(keyword.trim()) || status !== 'all' || hasManualTagFilter || sortBy !== 'end-asc' || quickView !== 'all'
  const activeFilterCount =
    Number(Boolean(keyword.trim())) +
    Number(status !== 'all') +
    Number(hasManualTagFilter) +
    Number(sortBy !== 'end-asc') +
    Number(quickView !== 'all')
  const selectedTagSummary = selectedTags.length ? `${selectedTags.length}개 선택됨` : '전체 태그'
  ...
  const savedViews: Array<{
  ...
  }> = [
    {
      ...
      query: { status: 'ongoing', tags: [], sort: 'end-asc', q: '', quick: 'all' as QuickViewKey },
    },
    {
      key: 'd3-focus',
      label: 'D-3 집중',
      description: '마감임박만 추리기',
      query: { status: 'all', tags: [], sort: 'end-asc', q: '', quick: 'urgent' as QuickViewKey },
    },
    {
      key: 'handover-track',
      label: 'Handover 트랙',
      description: '핸드오버 태그 중심',
      query: { status: 'all', tags: ['Handover'], sort: 'end-asc', q: '', quick: 'all' as QuickViewKey },
    },
    {
      key: 'interest-track',
      label: '관심태그만',
  ...
  function setQueryValue(key: 'status' | 'sort' | 'q' | 'quick', value: string) {
  ...
  function setTagFilter(nextTags: string[]) {
    const next = new URLSearchParams(searchParams)
    next.delete('tag')
    if (quickView === 'interest') {
      next.delete('quick')
  ...
  function toggleTagFilter(itemTag: string) {
    setTagFilter(
      selectedTags.includes(itemTag)
        ? selectedTags.filter((tagItem) => tagItem !== itemTag)
  ...
  function applySavedView(view: {
    ...
    if (view.tags.length) next.set('tags', view.tags.join(','))
    ...
    <section className={`${styles.pageShell} ${reduceMotion ? styles.reduceMotion : ''}`}>
      ...
      <div className={`stack-lg ${styles.mainColumn}`}>
        ...
        <section className={`controls ${styles.filters}`}>
          ...
          <label className={`${styles.filterField} ${styles.sortField}`}>
            정렬
          ...
          <button className={`button ghost align-end ${styles.resetButton}`} type="button" onClick={resetAllFilters}>
            필터 초기화
          </button>
          <label className={`${styles.filterField} ${styles.tagField}`}>
            <div className={styles.tagFieldHead}>
              <span>태그 필터</span>
              <small>{selectedTagSummary}</small>
            </div>
            <div className={styles.tagMultiPicker}>
              <button
                type="button"
                className={`${styles.tagMultiButton} ${selectedTags.length === 0 ? styles.tagMultiButtonActive : ''}`}
              ...
              {tagOptions.map((itemTag) => (
                <button
                  key={itemTag}
                  type="button"
                  className={`${styles.tagMultiButton} ${selectedTags.includes(itemTag) ? styles.tagMultiButtonActive : ''}`}
                  onClick={() => toggleTagFilter(itemTag)}
                >
                  {itemTag}
                </button>
              ))}
        ...
        {hasActiveFilter ? (
          <section className={styles.activeFilters} aria-label="현재 적용 필터">
            ...
            {(quickView === 'interest' ? [] : selectedTags).map((itemTag) => (
              <button
                key={itemTag}
                className={styles.activeFilterChip}
                type="button"
                onClick={() => setTagFilter(selectedTags.filter((tagItem) => tagItem !== itemTag))}
              >
                태그: {itemTag} ×
              </button>
            ))}
        ...
        {filtered.length ? (
          <section className={styles.listWrap}>
            {filtered.map((item) => {
              ...
              const toneIndex = getToneIndexBySlug(item.slug)
              const mediaToneClass = styles[`cardMediaTone${toneIndex}`]
              const isClosedHackathon = item.status === 'ended' || deadlineMeta.variant === 'closed'
              const hasRealThumbnail = Boolean(item.thumbnailUrl && !item.thumbnailUrl.includes('example.com'))
              const thumbnailSrc = hasRealThumbnail ? item.thumbnailUrl : getFallbackThumbnailBySlug(item.slug)
              ...
              return (
                ...
                >
                  <div
                    className={`${styles.cardMedia} ${isClosedHackathon ? styles.cardMediaClosed : ''} ${!hasRealThumbnail ? `${styles.cardMediaFallback} ${mediaToneClass}` : ''}`}
                    aria-hidden="true"
                  >
                    <img src={thumbnailSrc} alt="" loading="lazy" />
                    {isClosedHackathon ? (
                      <div className={styles.mediaEndedOverlay} />
                    ) : null}
                  </div>
                  ...
                  <div className={styles.tagsInline}>
                    {(item.tags || []).map((itemTag) => (
                      <button
                        key={itemTag}
                        className={`tag ${styles.tagButton} ${selectedTags.includes(itemTag) ? styles.tagButtonActive : ''}`}
                        type="button"
